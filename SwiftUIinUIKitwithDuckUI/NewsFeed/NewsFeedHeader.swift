...
protocol NewsFeedHeaderDelegate {
    func didTapButtonn(_ tag: Int, item: Profile)
}

class NewsFeedHeader: D_ListHeader<Profile> {
    
    // MARK:- Delegate
    var delegate: NewsFeedHeaderDelegate?
    
    // MARK:- Item
    override var item: Profile! {
        didSet {
    ...
    override func observe() {
        ...
        button0.reactive.tap.observeNext { [weak self] in
            ...
            strongSelf.delegate?.didTapButtonn(strongSelf.button0.tag, item: strongSelf.item)
            }.dispose(in: bag)
        
        button1.reactive.tap.observeNext { [weak self] in
            guard let strongSelf = self else { return }
            strongSelf.delegate?.didTapButtonn(strongSelf.button1.tag, item: strongSelf.item)
            }.dispose(in: bag)
    }
